//Maximize Widget
angular.module('app')
    .directive('widgetMaximize', function ($rootScope) {
        return {
            restrict: 'A',
            ...
            link: function (scope, el, attr) {
                el.on('click', function () {
                    ...
                    if (widget.hasClass("maximized")) {
                        ...
                        widget.find(".chart").css("height", "");

                        if(update) {
                            //Executa novamente a cada 5 minutos
                            clearInterval($rootScope.setIntervalFunction);
                        }
                    } else {
                        if (button) {
                        ...
                        if (widget) {
                            ...
                            widget.find(".widget-body").height(windowHeight - 2*headerHeight);
                            widget.find(".chart").height(windowHeight - 2*headerHeight);

                            if(update) {
                                //Executa novamente a cada 2 minutos
                                $rootScope.setIntervalFunction = setInterval(update, 120000);
                            }
                        }
                    }
                });
